...
del_folder__without_files = ["d fo", "fo d",
                             "delete folder without files", "folder delete"]
exit__func = ["exit", "bye", "leave", "nothing"]
same_cmd__multi_times = ["sc mt", "mt sc",
                         "same command multiple times", "mutiple commands"]

# <-- Add the list whenever more commands are added -->
all_cmd_entries = create__file + create__folder + \
    remove__file + remove__folder + move__file
...
def create_file(need_output, file_name):
    ...
    :param need_output: If you want the program to output the file name
    :param file_name: The name of the file you want to create
    """
    try:
...
def remove_file(need_output, file_name):
    """
    It removes a file if it exists, and prints a message if it doesn't

    :param need_output: If True, the function will print out the file name and whether it was removed or
    not
...
def move_folder(need_output, folder_name, folder_name_to):
    """
    It takes a folder name, and moves all the files in that folder to another folder.

    :param need_output: Whether or not to print the files being moved
    :param folder_name: The folder that the files are in
    :param folder_name_to: The folder you want to move the files to
    """
    files, contents = [], []
    if os.path.isdir(folder_name):
        ...
        for file_make in range(len(files)):
            file_open = open(
                f'{folder_name_to}/{folder_name}/{files[file_make]}', 'w+')
...
def delete_folder_without_files(need_output, folder_name):
    """
    It deletes the folder and moves all the files present in it to the present directory.

    :param need_output: If you want to see the output of the function, set it to True
    :param folder_name: The name of the folder you want to delete
    """
    files, contents = [], []
    if need_output:
...
def same_command__multiple_times(cmd, all_cmds=all_cmd_entries):
    ...
    try:
        times = int(
            input('Enter the amount of times you want to run the command: '))
    except ValueError:
...
def inner_execute(pre_cmd='', times=0, all_cmd_entries=all_cmd_entries, same_cmd__multi_times=same_cmd__multi_times):
    """
    If the pre_cmd is not empty, then set command_execute to False, set cmd to pre_cmd, set cmd_method
    to times, and set i to 0.

    :param pre_cmd: The command that was entered before the current command
    :param times: the number of times the command is executed, defaults to 0 (optional)
    :param all_cmd_entries: This is a list of all the commands that the user has entered
    :param same_cmd__multi_times: If the user wants to execute the same command multiple times, this is
    the number of times they want to execute it
    """
    ...
    if pre_cmd != '':
        ...
        cmd_method = times
        i = 0
    # The main function of the program. It takes the command from the user, and executes the
